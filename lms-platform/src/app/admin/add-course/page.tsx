  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'courses'), {
        ...
        curriculum, // Include curriculum
        createdAt: new Date()
      });

      setStatus({ type: 'success', message: 'Müfredat ve Kurs başarıyla eklendi!' });
        ...
        <div className="bg-[#1A1A37] px-8 py-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-[#FF7E31]/20 flex items-center justify-center">
              <span className="material-symbols-outlined text-[#FF7E31] text-2xl">architecture</span>
            </div>
            <div>
              <h1 className="text-2xl font-black text-white tracking-tight">Curriculum Builder</h1>
              <p className="text-slate-400 text-sm mt-1">Gelişmiş müfredat oluşturucu ve kurs yönetim paneli.</p>
            </div>
          </div>
          <button 
            type="button"
            onClick={(e) => {
              // Prevent default if it's inside a form natively
              // But here we'll manually fire handleSubmit or just do it inline
          ...
          >
            {isSubmitting ? 'Kaydediliyor...' : 'Tümü Kaydet'}
            {!isSubmitting && <span className="material-symbols-outlined text-[18px]">publish</span>}
          </button>
        </div>
